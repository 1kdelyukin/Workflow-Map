...
export function initPanel(host) {
  ...
  function render() {
    ...
    const linkCount = kids.length + edges.length;
    ...
    host.innerHTML = `
      ...
      <div class="panel-tabs" role="tablist">
        <button class="ptab" data-tab="details" role="tab">Details</button>
        <button class="ptab" data-tab="content" role="tab">Content${n.content?.trim() ? ' •' : ''}</button>
        <button class="ptab" data-tab="links" role="tab">Links${linkCount ? ` (${linkCount})` : ''}</button>
      </div>

      <div class="panel-scroll">
        <div class="panel-pane" data-pane="details">
  ...
  function wireTabs() {
    const apply = (tab) => {
      for (const b of host.querySelectorAll('.ptab')) b.classList.toggle('on', b.dataset.tab === tab);
      for (const pane of host.querySelectorAll('.panel-pane')) pane.hidden = pane.dataset.pane !== tab;
    };
    const saved = uiPref('panelTab');
    apply(['details', 'content', 'links'].includes(saved) ? saved : 'details');
    for (const b of host.querySelectorAll('.ptab')) {
      b.addEventListener('click', () => {
        apply(b.dataset.tab);
        uiPref('panelTab', b.dataset.tab);
      });
    }
  }
  ...
  on('selection', (opts = {}) => {
    // selections made while starting a drag are "quiet" — they don't open
    // the panel; a clean click re-emits loudly on pointerup
    if (opts.quiet && host.hidden) return;
    render();
    if (opts.focusTitle) {
